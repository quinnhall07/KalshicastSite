...
import { useLocations, useForecasts, useObservations, useAccuracyStats } from "@/hooks/use-weather";
...
export default function Dashboard() {
  ...
  const currentLocationId = selectedLocationId ? parseInt(selectedLocationId) : undefined;

  const { data: forecasts } = useForecasts(currentLocationId ? { locationId: currentLocationId } : undefined);
  const { data: observations } = useObservations(currentLocationId ? { locationId: currentLocationId } : undefined);
  const { data: stats, isLoading: loadingStats } = useAccuracyStats(currentLocationId);
      ...
      <header className="border-b border-white/5 bg-background/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-display font-bold tracking-tight">WeatherBet</h1>
              <p className="text-xs text-muted-foreground hidden sm:block">Predictive Model Accuracy Tracker</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <SelectContent>
                {locations?.map(loc => (
                  <SelectItem key={loc.id} value={loc.id.toString()}>
                    <span className="font-mono text-muted-foreground mr-2">{loc.code}</span>
                    {loc.name}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="text-right hidden sm:block">
            ...
            <p className="font-mono font-bold text-primary">Mean Absolute Error (MAE)</p>
          </div>
        </div>
        ...
        <Card className="glass-card p-6 border-white/10 min-h-[500px] flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Comparing predicted highs against actual recorded temperatures.</p>
            </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <Tooltip 
                  ...
                  labelStyle={{ marginBottom: '8px', color: '#aaa', fontSize: '12px' }}
                  labelFormatter={(date) => format(parseISO(date as string), 'EEEE, MMMM do, yyyy')}
                />
